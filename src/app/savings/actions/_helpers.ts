...
// Find the current user's personal space. RLS ensures we only ever see
// spaces the user is a member of, so this is safe.
export async function getPersonalSpace(supabase: SupabaseClient) {
  const { data, error } = await supabase
    ...
    .limit(1)
    .single();
  if (error || !data) throw new Error("Personal space not found");
  return data;
}
...
export async function fetchContributionContext(
  ...
  const { data } = await supabase
    ...
    .select("fund_id, month_id, months!inner(year, month, unlock_reason)")
