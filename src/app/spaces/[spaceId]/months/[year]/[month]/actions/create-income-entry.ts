...
export async function createIncomeEntry(
  ...
  try {
    ...
    // revalidate that month's path so it picks up the new row. Both
    // months are in the same space (getOrCreateMonth was called with
    // viewedMonthRow.space_id), so one spaceId covers both calls.
    revalidatePath(spaceMonthUrl(viewedMonthRow.space_id, year, month));
