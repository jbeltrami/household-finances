...
// amortizações mapped as Conta/Despesa rows to fold into the report totals,
// so the PDF matches the on-screen monthly view.
//
// Everything is computed AS OF month-end: only amortizações dated on or
// before then shape the schedule, and only parcelas due by then count as
// paid — so a report for a past month reflects that month, not today. That
// is why this rebuilds the schedule from `extras` rather than reading the
// hydrated one, which is the loan as it stands now.
export function buildFinancingReport(
  ...
  for (const { financing: f, extras: allExtras, paidNumbers: paidSet } of ledger) {
    ...
    // Paid as of month-end = paid parcelas whose scheduled date <= month-end.
    let paidCount = 0;
    ...
    // Skip financings already settled before this month (no parcela now and
    // nothing left owed) — they'd just be noise in later reports.
...
export async function getFinancingReport(
  ...
  return buildFinancingReport(
    await getFinancingLedger(supabase, spaceId),
    year,
    month
  );
}
